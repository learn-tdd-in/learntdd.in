...
To see how TDD works in React Native, let's walk through a simple real-world example of building a feature. We'll be using React Native 0.70, the [Jest][jest] test runner, and several testing libraries: [React Native Testing Library][react-native-testing-library] and [Jest Native][jest-native] for component tests and [Detox][detox] for end-to-end tests.
...
```sh
$ npx react-native init ReactNativeTDD --version 0.70.0
```
...
```sh
$ yarn add --dev @testing-library/react-native@11.0.0 \
...
Example: should show world screen after tap [FAIL]
 ...
 FAIL  e2e/firstTest.e2e.js (90.608s)
  Example
...
As our last setup step, let's clear out some of the default code to get a clean starting point. Delete `e2e/firstTest.e2e.js` and `__tests__`, and replace the contents of `App.js` with this app skeleton:
...
* ["Test-Driven Development in React Native"](https://www.youtube.com/playlist?list=PLXXnezSEtvNPZroRdvjhEVzOhURl572Lf) - a free eight-video series of live stream recordings. From 2018 so technical details have changed, but the approach still applies.
